...
{
	glActiveTexture(GL_TEXTURE0 + tOffset);
...
// https://www.khronos.org/opengl/wiki/Texture#Render_targets
GLuint loadTextureL(unsigned char *data,int width,int height ,int n, int tOffset)
{
	glActiveTexture(GL_TEXTURE0 + tOffset);
	glBindTexture(GL_TEXTURE_2D, Textures_array[tOffset]);
	glTexImage2D(GL_TEXTURE_2D,0,GL_RGB,width,height,0,GL_RGB,GL_UNSIGNED_BYTE,data);
...
int main(void)
{
	...
	int pW, pH = 715;  int pN = 4;
	int sW, sH = 1000; int sN = 4;
	unsigned char *dataP = stbi_load("textures/ground2.jpg", &pW, &pH, &pN, 0);
	unsigned char *dataS = stbi_load("textures/fire.jpg", &sW, &sH, &sN, 0);
	...
	for (int x = 0; x < 19; ++x)
	{
		for (int y = 0; y < 19; ++y)
		{
			...
			g_vertex_buffer_data[offset + 11] = 0.0f;
	...
	do {
		...
		GLint TexturePID = glGetUniformLocation(programID, "texture[0]");
		glUniform1i(TexturePID, 0);*/
		//glGenerateMipmap(GL_TEXTURE_2D);

		GLuint currentTexture = loadTextureL(dataP, pW, pH, pN, 0);

		glGenBuffers(1, &vertexbuffer);
		glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
		glBufferData(GL_ARRAY_BUFFER, sizeof(g_vertex_buffer_data), g_vertex_buffer_data, GL_STATIC_DRAW);

		glGenBuffers(1, &Textures_array[0]);
		glBindBuffer(GL_ARRAY_BUFFER, Textures_array[0]);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		GLint TextureSID = glGetUniformLocation(programID, "texture[1]");
		glUniform1i(TextureSID, 1);*/
		//glGenerateMipmap(GL_TEXTURE_2D);
		
		currentTexture = loadTextureL(dataS, sW, sH, sN, 1);
		...
		glGenBuffers(1, &Textures_array[1]);
		glBindBuffer(GL_ARRAY_BUFFER,Textures_array[1]);
		...
		*/
		...
		// Draw the triangle !
		glDrawArrays(GL_TRIANGLES, 2,  vertices.size()); // 3 indices starting at 0 -> 1 triangle
